...
public class TestCheckpoint extends TestCase {
  ...
  void testSecondaryFailsToReturnImage(Configuration conf)
    ...
    try {
      ...
      for (Iterator<StorageDirectory> it = 
         ...
         sd = it.next();
      assertTrue(sd != null);
      long fsimageLength = image.getStorage().getStorageFile(sd, NameNodeFile.IMAGE, 0xDEADBEEF).length();
      //
      // Make the checkpoint
      //
      SecondaryNameNode secondary = startSecondaryNameNode(conf);
  ...
  void testStartup(Configuration conf) throws IOException {
    ...
    File newestImageFile = findNewestImageFile(nameDirs.get(0).getPath() + "/current");
    ...
    for (StorageDirectory sd : image.getStorage().dirIterable(NameNodeDirType.IMAGE)) {
      ...
      File recreatedImageFile = findNewestImageFile(sd.getCurrentDir().toString());
  ...
  public void testSaveNamespace() throws IOException {
    ...
    try {
      ...
      //   edits_inprogress_1 -> edits_1-7  (finalized)
      //   fsimage_7 created
      //   edits_inprogress_8 created
      //
      for(URI uri : editsDirs) {
        File ed = new File(uri.getPath());
        ...
        File finalizedEdits = new File(ed, "current/edits_1-7");
        assertTrue(finalizedEdits.exists());
        assertTrue(finalizedEdits.length() > Integer.SIZE/Byte.SIZE);

        assertTrue(new File(ed, "current/edits_inprogress_8").exists());
      }
      
      Collection<URI> imageDirs = cluster.getNameDirs();
      for (URI uri : imageDirs) {
        File imageDir = new File(uri.getPath());
        File savedImage = new File(imageDir, "current/fsimage_7");
